...
const SearchBox: React.FC<SearchBoxProps> = ({ onSearchOptionsChange: handleSearchOptionsChange }) => {
   ...
   const { data: events } = useGetEventsQuery({ pagination: false, limit: 5, search: searchEvent })
   const { data: users } = useGetUsersQuery({ pagination: false, limit: 5, search: searchEvent })
   const form = useForm<Partial<FormValue>>()

   const handleSearch = (data: FormValue) => handleSearchOptionsChange(data)
   ...
   return (
      <Box className='group relative w-full rounded-lg border @xs:basis-full'>
         ...
         <Input
            className='border-none px-8 group-focus:border-primary'
         ...
         <Popover>
            ...
            <PopoverContent align='center' className='w-96 xl:w-fit' sideOffset={12}>
               <Form {...form}>
                  <SearchForm onSubmit={form.handleSubmit(handleSearch)}>
                     ...
                     <Box className='col-span-2 mt-6 flex justify-end'>
